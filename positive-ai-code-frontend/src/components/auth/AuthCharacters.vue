<template>
  <div class="auth-characters-scale">
    <div ref="containerRef" class="auth-characters" aria-hidden="true">
...
const props = withDefaults(defineProps<AuthCharactersProps>(), {
  isTyping: false,
  showPassword: false,
  passwordLength: 0,
...
onMounted(() => {
  ...
  gsap.set(containerRef.value.querySelectorAll('.pupil'), { x: 0, y: 0 })
  gsap.set(containerRef.value.querySelectorAll('.eyeball-pupil'), { x: 0, y: 0 })
  ...
  const tick = () => {
    ...
    const { isTyping, isHidingPassword: hiding, isShowingPassword: showing, isLooking, isPasswordGuardMode } =
      stateRef.value
    ...
    if (purpleRef.value && !showing && !isLooking) {
      const pp = calcPos(purpleRef.value)
    ...
    if (blackRef.value && !showing && !isLooking) {
      const bp = calcPos(blackRef.value)
...
watch(
  () => [isHidingPassword.value, isShowingPassword.value, props.isPasswordGuardMode] as const,
  () => {
    if (props.isPasswordGuardMode) {
      applyPasswordGuardMode()
